...
def main(argv):
    ...
    if inputfile == '':
        ...
        print("-----------------------", "\n\n")
        ready = input("Recordá agregar un espacio a todas las celdas de la columna R. Poné 1 para continuar: ")
        if ready == "1":
            ...
            for value in sheet.iter_rows(min_row=2, min_col=15, max_col=15, values_only=True):
                if value[0] != None:
                    ...
                    percent = result[1]
                    if percent > 80: #and sheet["R" + str(res_counter)] != None and sheet["S" + str(res_counter)] == None:
            ...
            for value in sheet.iter_rows(min_row=2, min_col=16, max_col=16, values_only=True):
                if value[0] != None:
                    result = process.extractOne(
                        value[0], server_names, scorer=fuzz.partial_ratio)
                    percent = result[1]
                    if percent > 80: #and sheet["T" + str(det_counter)] == None and sheet["U" + str(det_counter)] == None:
